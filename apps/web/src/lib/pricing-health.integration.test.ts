...
test('transit pagination reaches every provider; health separates task success, full coverage and current anomalies', {skip:!adminUrl}, async()=>{
 ...
 try{
  ...
  const runs:string[]=[];
  for(const [status,complete] of [['success',true],['success',false],['failed',false],['running',false]] as const){
  ...
  assert.equal(health.runSuccessRate,2/3);assert.equal(health.runningRuns24h,1);
