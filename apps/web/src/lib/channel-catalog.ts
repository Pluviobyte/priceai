...
import { CHANNEL_MODES, type ChannelFilters } from "./channel-filters";
...
const BASE = `with published as (
  ...
  select o.id::text, cp.slug product_slug, cp.display_name product_name, cp.brand platform,
    m.slug merchant_slug, m.name merchant_name, ros.raw_title, o.offer_mode,
...
export async function getChannelCatalog(filters: ChannelFilters, read: typeof query = query): Promise<ChannelCatalog> {
  ...
  const selection = filters.view === "products"
    ...
    : filters.view === "merchants"
      ? `select merchant_slug id, merchant_slug,merchant_name,count(*)::int offer_count,
  ...
  const order = filters.sort === "price" && filters.view !== "merchants"
