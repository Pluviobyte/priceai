...
import { compareCollectedSubscriptionPrices, getCachedOfficialSubscriptionPrices, isFreshOfficialSubscriptionPrice, hasCurrentCnyEstimate, type OfficialSubscriptionPrice } from "./public-pricing";
...
export function buildHomeBaseline(prices: OfficialSubscriptionPrice[], offers: HomeOffer[]): BaselineRow[] {
  return PLACEHOLDER.baseline.map(base => {
    ...
    // At one currency and amount the official site outranks an app store, whichever was swept last.
    const floor = [...officialPrices].sort((a,b) => compareCollectedSubscriptionPrices(a,b)
