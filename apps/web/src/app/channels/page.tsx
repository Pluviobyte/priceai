...
const categoryGlyphs: Record<string, string> = { video: "/category-icons/video.svg", mail: "/category-icons/mail.svg", verification: "/category-icons/verification.svg", other: "/category-icons/other.svg" };
...
function Results({ data, filters, view }: { data: ChannelCatalog; filters: ChannelFilters; view: CatalogView }) {
  ...
  return <ul className={styles.quoteList} aria-label={expanded ? "原始商品报价" : "同规格参考报价"}>
    {data.rows.map(row => <li className={styles.quoteRow} key={`${row.id}-${row.currency}`}>
      <div className={styles.product}><ProductMark platform={row.platform} /><div><Link href={`/products/${row.product_slug}`}><b>{expanded ? row.raw_title : row.product_name}</b></Link>{expanded ? <small><Link href={`/merchants/${row.merchant_slug}`}>{row.merchant_name}</Link> · {row.product_name}</small> : <small>{row.platform}{row.lowest_merchant_name ? ` · 最低来自 ${row.lowest_merchant_name}` : ""}</small>}</div></div>
      <div className={styles.quoteFacts}><OfferFacts row={row} /></div>
      <div className={styles.quotePrice}><span className={styles.quoteLabel}>{expanded ? "原站报价" : "有货参考最低价"}</span><strong className={styles.price}>{channelMoney(row.price, row.currency)}</strong><small>{row.currency} · {row.duration_days ? `${row.duration_days} 天总价` : "期限待确认"}</small>{expanded && !row.available && <small>不计入有货最低价</small>}{!expanded && row.price === null && <small>暂无有货且期限明确的报价</small>}{!expanded && row.price !== null && <small>{row.warranty_price ? `有质保最低 ${channelMoney(row.warranty_price, row.currency)}` : "该规格暂无带质保报价"}</small>}{!expanded && row.lowest_raw_title && <small className={styles.lowestTitle}>{row.lowest_raw_title}</small>}</div>
      <div className={styles.quoteStock}>{expanded ? <><OfferStatus row={row} /><small>{row.stock_count === null ? "数量未提供" : `数量 ${row.stock_count}`}</small></> : <><b>{row.merchant_count} 家 · {row.offer_count} 条</b><small>{row.available_count} 条已确认有货{row.unavailable_count ? ` · ${row.unavailable_count} 条缺货或待复核` : ""}</small></>}</div>
