...
function CompareLink({ row, filters }: { row: ChannelRow; filters: ChannelFilters }) {
  return <Link className={styles.button} href={channelHref(filters, { view: "compare", group: "expanded", spec: row.spec_key })}>比较这 {row.offer_count} 条报价 →</Link>;
}
...
function Results({ data, filters, view }: { data: ChannelCatalog; filters: ChannelFilters; view: CatalogView }) {
  ...
  return <div className={styles.tableWrap} tabIndex={0} role="region" aria-label="订阅比价表，可横向滚动"><table className={styles.table}>
    ...
    <thead><tr><th scope="col">{expanded ? "原始商品 / 商家" : "订阅商品"}</th><th scope="col">交付与规格</th><th scope="col">{expanded ? "原站报价" : "有货参考最低价"}</th><th scope="col">{expanded ? "库存状态" : "商家 / 报价"}</th><th scope="col">最近核验（北京时间）</th><th scope="col">操作</th></tr></thead>
    <tbody>{data.rows.map(row => <tr key={`${row.id}-${row.currency}`}>
      <td><div className={styles.product}><ProductMark platform={row.platform} /><div><Link href={`/products/${row.product_slug}`}><b>{expanded ? row.raw_title : row.product_name}</b></Link>{expanded ? <small><Link href={`/merchants/${row.merchant_slug}`}>{row.merchant_name}</Link> · {row.product_name}</small> : <small>{row.platform}</small>}</div></div></td>
      <td className={styles.facts}><OfferFacts row={row} /></td>
      <td><strong className={styles.price}>{channelMoney(row.price, row.currency)}</strong><small>{row.currency} · {row.duration_days ? `${row.duration_days} 天总价` : "期限待确认"}</small>{expanded && !row.available && <small>不计入有货最低价</small>}{!expanded && row.price === null && <small>该组暂无有货且期限明确的报价</small>}</td>
      <td>{expanded ? <><OfferStatus row={row} /><small>{row.stock_count === null ? "数量未提供" : `数量 ${row.stock_count}`}</small></> : <><b>{row.merchant_count} 家 · {row.offer_count} 条</b><small>{row.available_count} 条已确认有货</small></>}</td>
      <td><time dateTime={row.verified_at ? new Date(row.verified_at).toISOString() : undefined}>{channelTime(row.verified_at)}</time></td>
      <td className={styles.actions}>{expanded ? <>{row.available ? <a className={styles.button} href={`/out/${row.id}`} target="_blank" rel="noopener noreferrer nofollow">前往原站 ↗</a> : <Link className={styles.button} href={`/products/${row.product_slug}?stock=all`}>看同款其他报价 →</Link>}{row.risk_facts.length > 0 && <details className={styles.risks}><summary>{row.risk_facts.length} 项需注意</summary><ul>{row.risk_facts.map(fact => <li key={fact}>{fact}</li>)}</ul></details>}<ReportOffer row={row} /></> : <CompareLink row={row} filters={filters} />}</td>
    </tr>)}</tbody>
  </table></div>;
}
  ...
  return <div className="priceai-page"><main className={styles.shell}>
    ...
    {hasFilters && <div className={styles.activeFilters}>
      {filters.spec && <p className={styles.specLock}><span className={styles.specLockLabel}>已锁定规格</span>{data.spec
        ...
        : <span className={styles.specLockBody}>该规格当前没有在售报价</span>}<span className={styles.specLockHint}>{filters.view === "merchants" ? "只看有这一规格报价的商家" : "只看这一规格的报价"}</span><Link className={styles.chipClear} href={channelHref(filters, { spec: "" })}>✕ 取消锁定</Link></p>}
