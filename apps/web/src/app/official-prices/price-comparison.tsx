...
function Quote({ row }: { row: OfficialSubscriptionPrice }) {
  ...
  return <div className={styles.quote}>
    <span className={styles.channel}>{channels[row.channel] ?? row.channel}{!isFreshOfficialSubscriptionPrice(row) && <em>待更新</em>}</span>
    ...
    <time dateTime={row.verifiedAt.toISOString()}>核验 {row.verifiedAt.toLocaleDateString("zh-CN", { timeZone: "Asia/Shanghai", month: "2-digit", day: "2-digit" })}</time>
    ...
    {!available ? <p role="status">暂时无法读取价格数据，请稍后刷新重试。</p> : !plans.length || !visibleRegions.length ? <p role="status">没有匹配的套餐或地区，请重置筛选。</p> : <div className={styles.scroll} tabIndex={0} role="region" aria-label="订阅价格对照表，可左右滚动">
      <table className={styles.table}>
        ...
        <tbody>{plans.map((plan) => <tr key={`${plan.vendor}:${plan.planCode}:${plan.billingPeriod}`}>
          ...
          {visibleRegions.map((item) => { const quotes = index.get(`${plan.vendor}:${plan.planCode}:${plan.billingPeriod}:${item.code}`) ?? []; return <td key={item.code}>{quotes.length ? [...quotes].sort((a, b) => Object.keys(channels).indexOf(a.channel) - Object.keys(channels).indexOf(b.channel)).map((row) => <Quote key={row.id} row={row} />) : <span className={styles.missing}>尚无报价</span>}</td>; })}
        </tr>)}</tbody>
      </table>
    </div>}
    <p className={styles.note}>人民币金额按记录所关联的汇率估算，未另加税费及跨境支付手续费。“待更新”保留上次核验价格；“尚无报价”表示尚未收录，不代表免费或不能购买。</p>
  </section>;
}
