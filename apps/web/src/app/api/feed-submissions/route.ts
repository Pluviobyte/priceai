...
export async function POST(request: Request) {
  ...
  try {
    ...
    const id = await createMerchantFeedApplication({ ...required, ...(notes ? { notes } : {}), accountOwnerKey: account ? accountKey(account) : null, fingerprint: submissionFingerprint(request) });
