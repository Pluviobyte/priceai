...
import { createIceFlameMaterial, updateIceFlameMaterial } from "./ice-flame-material";
...
const CORE_SIZE = 0.5;
const FLAME_SCALE = new THREE.Vector3(0.72, 0.92, 0.72);
const DECAL_SIZE = 0.92;
const DECAL_LIFT = 0.035;
const USE_SOLID_ICE_CORE = process.env.NEXT_PUBLIC_ICE_CORE_VARIANT === "solid";
...
function drawingPixelRatio() {
  ...
  return Math.min(Math.max(dpr * zoom, 1), USE_SOLID_ICE_CORE ? 4 : 2.5);
}
...
export function IceCubeLogo({ size }: IceCubeLogoProps = {}) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const applyTheme = () => {
        ...
        renderer.toneMappingExposure = dark ? 0.98 : 0.88;
        for (const material of flameMaterials) {
          updateIceFlameMaterial(material, material.uniforms.uTime?.value ?? 0, material.uniforms.uIntensity?.value ?? 0.92, dark);
        }
      };
      ...
      const flameGeometry = USE_SOLID_ICE_CORE ? undefined : new THREE.BoxGeometry(1, 1, 1);
      const shellMaterial = shellGlassMaterial();
      const coreMaterial = USE_SOLID_ICE_CORE ? iceCoreMaterial() : undefined;
      const iconMaterials = MODEL_ICON_ENTRIES.map(() => decalMaterial());

      new HDRLoader().load(STUDIO_HDR_URL, (hdr) => {
        ...
        const pmrem = new THREE.PMREMGenerator(renderer);
        const studio = pmrem.fromScene(envScene, 0.035);
        hdr.dispose();
        pmrem.dispose();
        ...
        if (coreMaterial) coreMaterial.envMapIntensity = 0;
      ...
      for (const z of coordinates) {
        for (const y of [...coordinates].reverse()) {
          for (const x of coordinates) {
            ...
            iceGeometries.push(shellGeometry);

            if (USE_SOLID_ICE_CORE && coreMaterial) {
              ...
              iceGeometries.push(coreGeometry);
              cubelet.add(new THREE.Mesh(coreGeometry, coreMaterial));
            } else if (flameGeometry) {
              const flameMaterial = createIceFlameMaterial(seed, isDarkTheme());
              const flame = new THREE.Mesh(flameGeometry, flameMaterial);
              flame.scale.copy(FLAME_SCALE);
      ...
      const coreLight = new THREE.PointLight(0x7ec8ee, 0.17, 3.2, 2.0);
      ...
      let engagement = 0;
      const brandLink = canvas.closest<HTMLElement>(".site-brand");
      ...
      const renderFrame = () => {
        ...
        yawRotation += delta * (0.3 + engagement * 0.1);
        cube.rotation.x = -0.4;
        ...
        const flameIntensity = 0.92 + engagement * 0.1;
        for (const material of flameMaterials) {
          updateIceFlameMaterial(material, motionTime, flameIntensity, isDarkTheme());
        }
        coreLight.intensity = 0.17 + engagement * 0.05 + Math.sin(motionTime * 2.1) * 0.012;
  ...
  return (
    <span
      className="priceai-ice-cube-logo"
      aria-hidden="true"
      style={size ? { width: size, height: size, flexBasis: size } : undefined}
    >
      <span className="priceai-ice-cube-logo-glow" />
      {!failed && <canvas ref={canvasRef} className="priceai-ice-cube-logo-canvas" />}
      {failed && (
        <span className="priceai-ice-cube-logo-fallback">
