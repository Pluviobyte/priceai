...
export default async function GenerationsPage() {
  ...
  return <main className="admin-shell"><header className="admin-header"><div><span className="section-kicker">Atomic publication</span><h1>发布与回滚</h1></div><AdminNav /></header>
    <p className="listing-lead">保留中的版本可以回滚，过期版本仅保留发布记录。回滚会在单一数据库事务中恢复报价并切换 latest 指针。</p>
    ...
    <div className="review-list">{generations.map((generation) => <article className="review-card" key={generation.id}><div className="review-facts"><span>{generation.isCurrent ? "当前线上" : generation.status}</span><span>{time(generation.publishedAt)}</span><span>{generation.id}</span></div><h3>{generation.offerCount} 条报价 · {generation.productCount} 个产品 · {generation.sourceCount} 个来源</h3><div className="review-facts"><span>快照 {generation.snapshotState === "retained" ? generation.snapshotCount : "已过期"}</span><span>新增 {generation.addedCount ?? "未记录"}</span><span>移除 {generation.removedCount ?? "未记录"}</span><span>变化 {generation.changedCount ?? "未记录"}</span><span>{generation.manifestHash ? `Manifest ${generation.manifestHash.slice(0, 12)}` : "尚无对象 Manifest"}</span></div>{!generation.isCurrent && generation.snapshotState === "retained" && ["published", "superseded"].includes(generation.status) && generation.snapshotCount > 0 && <form className="anomaly-form" method="post" action={`/api/admin/generations/${generation.id}`}><input name="reason" minLength={4} maxLength={500} required placeholder="输入回滚原因" /><button className="danger" type="submit">回滚到该代</button></form>}</article>)}</div>
  </main>;
}
