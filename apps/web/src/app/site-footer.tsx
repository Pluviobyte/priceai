...
export function SiteFooter() {
  return <footer className="priceai-footer"><nav aria-label="站点底部导航"><Link href="/guides">▣　入门指南</Link><Link href="/commercial">♧　商业合作</Link><Link href="/support">♡　支持作者</Link></nav><section className="priceai-sponsors" aria-label="底部赞助展示区广告位"><div className="priceai-sponsor-head"><h2>赞助商列表</h2><Link href="/commercial#slots">成为赞助商</Link></div><div className="priceai-sponsor-grid">{sponsors.map(([name,text,label])=><a href="/commercial" key={name}><span className="priceai-sponsor-image" /><div className="priceai-sponsor-copy"><div><h3>{name}</h3><em>{label}</em></div><p>{text}</p></div></a>)}</div></section></footer>;
}
