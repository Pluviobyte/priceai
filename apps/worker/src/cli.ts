...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Classification is written at publish time, so a rules change only reaches the
    // catalogue on the next publication. This republishes the snapshots already
    // collected, without re-crawling a single shop.
    if (command === 'publish') {
      await seedCanonicalProducts(database.db);
      console.log(JSON.stringify({ event: 'channels_published', ...(await publishLatestSnapshots(database.db)) }));
      return;
    }
