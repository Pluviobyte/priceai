...
## Collector recovery confirmed — Sep 18, 2026 15:25 CST

This update supersedes the paused state in the preceding historical entry. The Hangzhou relay and DMIT tunnel were restarted at approximately 07:14 UTC and remained active after about ten minutes of observation. Hangzhou retains a 15-second request interval; the original 2.5-second speed has not been restored. Both dependency services remain enabled.

The earlier automatic collection pause based solely on host I/O wait above 10% was too conservative: host I/O includes other projects and does not by itself demonstrate service impact. The recovery observation instead tracked container continuity, existing health checks, memory and disk headroom, while recording I/O for attribution. Across 60 samples covering 21 containers, no restart, exit, OOM flag or existing health degradation was detected. Eight containers have health checks; the others were checked only for running state. This does not establish zero business latency impact.

By 07:24 UTC, nine full source collections (eight via Hangzhou) and seven publications completed; publication duration was 36.2–44.6 seconds. Database source timestamps and complete-run pointers were verified. Of 716 enabled sources, 230 had full success in the preceding 24 hours; 486 remained overdue. No publication failure was observed, and Hangzhou had no active WAF cooldown. Temporary high-frequency observers exited normally; the collection services remained running.

A follow-up at 08:57 UTC confirmed both services still active, 296 sources with full success in the preceding 24 hours, and 83 completed publications since recovery. There were four collection task timeout/network errors followed by further successful collections, with no publication failures. The existing 16688 goods-list-disabled maintenance errors remain. Public Web/database readiness was healthy, and other container identities/start times remained unchanged.

Historical snapshot deletion and vacuum tuning are still not enabled. Permanent dependency-service and collection-freshness alerts remain outstanding. The existing periodic production checks do not cover these dependencies; the completed observation is not ongoing monitoring.
