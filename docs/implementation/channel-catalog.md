...
商家标签默认卡片展示，`layout=table` 切换表格；`scope=merchants` 兼容进入同一标签，后续链接规范为 `view=merchants`。模型快捷筛选、关键词、排序和分页均保存在 URL，切换卡片/表格保留当前页。搜索支持店铺名、入口域名与路径、商品名。卡片显示当前筛选内的商品覆盖、有货数、低价表现、核验时间，以及报价详情与原店铺入口。外链只允许无用户名密码的 HTTP(S) URL。

低价表现实时从当前发布批次计算，不引用外站商家评分。先取每商家每规格的有货最低价，再在完整目录中按规格排名；搜索商家不会移除竞争者。仅期限明确且至少两家商家有有效报价的规格计入分母，同店重复报价不增加计数。排名使用 `rank()`，同价并列，最低价为第 1 名，前五价为名次不大于 5。全部筛选确定展示哪些规格，排名仍采用全目录同规格的竞争者。未知、过期、缺货报价不参与低价统计。

`sort=low_price` 按最低价规格数、前五价规格数、可比规格数、有货数和新鲜度依次排序；默认仍按最近核验排序。无可比规格显示“暂无可比规格”。这些指标不构成信誉评分或担保。暂不展示缺少独立依据的公开运营时长、平台售后保证或自建卡网分类。
