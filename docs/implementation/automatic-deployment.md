...
管理员可启动 `scripts/priceai-release-observer.py`：每5秒检查发布开始前已经运行的其他容器（包括PriceAI数据库）是否重启、退出或失去健康，并读取宿主机内存、磁盘和CPU/iowait。只排除本次预期滚动更新的三个PriceAI应用。观察器不控制或重启其他服务。

心跳写入 `/run/priceai-release-observer.json`，权限0600。文件存在时，网关在拉取镜像前后、每个应用部署前及等待部署/健康过程中检查心跳；超过20秒或不健康立即停止推进。已经发出的Docker拉取或Dokploy滚动更新不会因此自动撤销，管理员需查看当前步骤；不得把门槛描述成对进行中操作的强制取消。没有安装健康检查的容器只验证运行/重启状态，不等于业务端到端可用性。

初期停止门槛：可用内存不足1GiB、磁盘不足20GiB、采样CPU忙碌超过85%或iowait超过10%。其他服务状态变化会锁存，恢复后也不自动解除。部署与后续观察结束后，管理员停止观察器，再移除其心跳文件以解除临时门槛；观察器意外退出则保留拒绝发布状态。
