...
- Shop API 准入完整试采最多12页；超出则等待审核，不把部分结果当完整目录。其他适配器保持原上限。
...
`/channels` 默认 AI 订阅与账号；`/channels?catalog=resources` 展示邮箱、验证服务等周边及 API 额度。商品范围随筛选/分页保存。未知期限或交付方式的商品不合并成可比较最低价。
